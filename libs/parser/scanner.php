<?php
	...
	if (count($ids)>1):
		...
		$postfix = implode("_", $ids);
		$suffix = "mass_".$postfix;
		$url="Массовое сканирование";	
		...
		foreach($ids as $key=>$val):			
			...
			$count_of_patterns += DB::GetOne("	SELECT count(id)
												...
												WHERE f_project_name=".$val);
			
		    $key=md5(BILLING_TOKEN.$val);	
	...
	if (($count_of_keywords>0) && ($count_of_patterns>0)):
		...
		foreach($ids as $k=>$id):
			...
			$search_engines = array_slice(explode("__", $se_pattern["f_search_engines"]),1,-1);
			foreach($search_engines as $key=>$val):
				...
				$keywords = DB::GetAll("	SELECT t1.id, t1.f_keyword, t3.f_project_url as url, t2.position, IFNULL(t2.position,0) as position
											...
											WHERE t1.f_project=%d AND t1.active=%d
